...
import quiz1_1 from '../../convert/generate/quiz1/quiz1';
import quiz1_2 from '../../convert/generate/quiz1/quiz2';
import quiz1_3 from '../../convert/generate/quiz1/quiz3';
...
function App() {
  ...
  const calculateTimer = (quiz) => {
    if (!quiz || !quiz.questions) return 3600; // 默认1小时

    const questionCount = quiz.questions.length;
    let timePerQuestion = 40; // 默认单选题40秒

    // 根据题型调整时间
    if (quiz.quizTitle && quiz.quizTitle.includes('多选题')) {
      timePerQuestion = 80; // 多选题每题80秒
    } else if (quiz.quizTitle && quiz.quizTitle.includes('判断题')) {
      timePerQuestion = 20; // 判断题每题20秒
    }
  ...
  const renderQuiz = () => (
    <div>
      ...
      <Quiz
        ...
        disableSynopsis
        timer={calculateTimer(selectedQuiz)}
        allowPauseTimer
        enableProgressBar
      />
    </div>
